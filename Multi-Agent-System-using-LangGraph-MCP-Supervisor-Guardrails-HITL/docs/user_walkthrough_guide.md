...
### What Happens Behind the Scenes
- **Jinja2 Template Delivery:** FastAPI's `GET /` route serves [`templates/index.html`](templates/index.html) with zero external build-step overhead.
- **State Hydration in Client JS:** [`static/script.js`](static/script.js) checks browser `localStorage` for any existing `travel_thread_id`. If none exists, the client prepares to receive a newly generated unique session UUID.
...
- **Weather Agent Execution:** Interacts directly with the local microservice [`custom_weather_mcp_server.py`](custom_weather_mcp_server.py). The server runs FastMCP over `stdio` and executes `@mcp.tool() get_current_weather(city="Tokyo")`.
...
4. **Resumption Dispatch:** When you click **Revise Using Feedback**, [`script.js`](static/script.js) sends a `POST /api/travel/approve` payload:
   ```json
   {
